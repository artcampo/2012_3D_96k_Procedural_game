...
{    
  ...
  //  Compute
  /*
  float ao1 = aoSampleLevel(pWS, nWS, 2, Input.Tex, 1, 4);
  float ao2 = aoSampleLevel(pWS, nWS, 4, Input.Tex, 2, 8);  
  float ao3 = aoSampleLevel(pWS, nWS, 8, Input.Tex, 4, 12);    
  float ao4 = aoSampleLevel(pWS, nWS, 10, Input.Tex, 5, 16);      
*/
  float ao1 = aoSampleLevel(pWS, nWS, 2,  Input.Tex, 2,  4);
  float ao2 = aoSampleLevel(pWS, nWS, 4,  Input.Tex, 4,  8);  
  float ao3 = aoSampleLevel(pWS, nWS, 8,  Input.Tex, 8,  12);    
  float ao4 = aoSampleLevel(pWS, nWS, 10, Input.Tex, 12, 16);   
  
    float finalAo = min( ao1, min(ao2, min(ao3, ao4 ) ) );
    /*
  finalAo  = ao1 + ao2 + ao3 + ao4 ;
  finalAo /=5;
    finalAo  = ao1 * ao2 * ao3 *ao4;
  */
