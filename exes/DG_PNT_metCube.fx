...
PS_OUTPUT PS( VS_OUTPUT input ) : SV_Target
{
  ...
  noiseIn = objectScale*input.posLS.yzx*float3(1.00000f, 1.00000f, 1.00000f) + float3(0.07000f, 0.07000f, 0.07000f);
  vectorModulus = float3(0.38462f, 0.38462f, 0.38462f);
  vectorValue = fmod( abs(noiseIn), vectorModulus );
  vectorValue -= 0.5*vectorModulus;
  vectorValue.y = 0;
  noiseT = 0;
  value2 = 0.38462f*sqrt(2);
  value =  procLine( vectorValue, float3(0.38462f, 0.00000f, 0.38462f), value2);
  value *=  procLine( vectorValue, float3(-0.38462f, 0.00000f, 0.38462f), value2);
  value *=  procLine( vectorValue, float3(0.38462f, 0.00000f, 0.00000f), value2);
  value *=  procLine( vectorValue, float3(0.00000f, 0.00000f, 0.38462f), value2);
  ...
  noiseT = Snoise3D ( noiseIn);
  float4 Signal00 = lerp( float4(1.00000f, 0.96471f, 0.69804f, 1.00000f), float4(1.00000f, 0.82353f, 0.41176f, 1.00000f), noiseT );

  noiseIn = objectScale*input.posLS.yzx*float3(1.40000f, 1.40000f, 1.40000f);
    inPerlin = noiseIn;
  divPerlin = 1;
  for( int i = 0; i < 4; ++i)
  {
...
//RGBA CHANNEL
  ...
  float4 Layer44 = 0.5*Signal00 + 0.5*Signal33;
