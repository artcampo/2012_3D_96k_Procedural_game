#include "SphereModel_PN.hpp"

using namespace std;

SphereModel_PN::SphereModel_PN( int numberVerticesPerEdge)
{
  mModel = new DxMeshModelBatch<VertexPosNor>( sizeof(VertexPosNor) , D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST);  
  std::vector<VertexPosNor> points = sphere(numberVerticesPerEdge);

  DxMeshModel<VertexPosNor> *model  = new DxMeshModel<VertexPosNor>( sizeof(VertexPosNor), points.size(), &points[0] );
  mModel->addMeshModel( model );
  mModel->createVB();

}

SphereModel_PN::~SphereModel_PN()
{
}

void SphereModel_PN::display()
{
  mModel->DisplaySansShader();
}


std::vector<VertexPosNor> SphereModel_PN::sphere(int size) {
    ...
    vector<VertexPosNor>              ret( num_vertex );
    vector<VertexPosNor>              ver( vertex_triangles_cube );
