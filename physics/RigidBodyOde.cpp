...
/*
    if ( mBodies[i]->mIsInRestingContact )         
    {
      ...
      D3DXVECTOR3 contactPosition = //rigidBodys[i]->xWS + 
          -mBodies[i]->mContactNormal[0]*mBodies[i]->mBodyExt->mBoundingSphereRadius;
      D3DXVec3Cross ( &rotationVector, &contactPosition, &rigidBodys[i]->vWS );
      //rigidBodys[i]->lWS += (rotationVector - rigidBodys[i]->lWS) *0.3f ;
      rigidBodys[i]->lWS = rotationVector ;
    }*/
    ...
    {      
      ...
      for ( int cpoint = 0; cpoint < mBodies[i]->mContactNormal.size(); ++cpoint)      
      {
  ...
  #ifndef _DEBUG_HIDE_FORCES
            *Global::mFileDebugBody[i]              
              << " F contact:: " << counterForce.x << ", " << counterForce.y << ", " << counterForce.z
              << std::endl;
  #endif
  #ifdef _DEBUG_LOG_BODIES_EXTENDED
            *Global::mFileDebugBody[i]              
              << " with n: " << mBodies[i]->mContactNormal[cpoint].x << ", " << mBodies[i]->mContactNormal[cpoint].y << ", " << mBodies[i]->mContactNormal[cpoint].z 
              << " with f: " << rigidBodys[i]->fWS.x << ", " << rigidBodys[i]->fWS.y << ", " << rigidBodys[i]->fWS.z 
              << " with dot: " << dot 
              << std::endl;
  #endif
#endif
        ...
        //if ( fabs(proj) < mContactDotEpsilon )
        //if ( proj < mContactDotEpsilon )
        {          
