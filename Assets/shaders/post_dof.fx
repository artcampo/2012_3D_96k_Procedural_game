...
{    
  ...
  float4 vColor = textColBase.Sample( samLinear, vSamplePosition) * kernel5[0];    
  for ( int i = 1; i < 5; ++i)
  {
    vColor.xyz += (textColBase.Sample( samLinear, vSamplePosition + float2( offset*off5[i], 0 ))*kernel5[i]).xyz;    
    vColor.xyz += (textColBase.Sample( samLinear, vSamplePosition - float2( offset*off5[i], 0 ))*kernel5[i]).xyz;    
  }
...
{    
  ...
  float4 vColorBlur = textColBlurred.Sample( samLinear, vSamplePosition) * kernel5[0];    
  for ( int i = 1; i < 5; ++i)
  {
    vColorBlur.xyz += (textColBlurred.Sample( samLinear, vSamplePosition + float2( 0,offset*off5[i] ))*kernel5[i]).xyz;    
    vColorBlur.xyz += (textColBlurred.Sample( samLinear, vSamplePosition - float2( 0,offset*off5[i] ))*kernel5[i]).xyz;    
  }
