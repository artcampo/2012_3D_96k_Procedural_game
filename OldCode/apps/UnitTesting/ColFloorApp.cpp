#include "ColFloorApp.hpp"


ColFloorApp::ColFloorApp(HINSTANCE& handler, int width, int height, bool isFullScreen, 
...
ColFloorApp::~ColFloorApp()
{
debugEnd();
}

Mesh*  ColFloorApp::loadMesh ( const char* aFileName )
{
  Mesh* mesh = new Mesh();
...
bool ColFloorApp::init()
{
  ...
  float a = 3000.0f;
  v.pos = D3DXVECTOR3(+a,0,-a); vert.push_back(v);  vertOnlyV.push_back (v.pos);
...
//////////////////////////////////////////////////////////
  ...
  D3DXVec3Cross( &mLastCrossProd, &mLastVelocityNonZero, &mLastNormalOfContact );

  GlobalGame::gShip = GlobalGame::gPhysic->createRigidBody( D3DXVECTOR3( 0, 2.0f, 0), 1.0f );
...
void ColFloorApp::onResize()
{
	BaseDX::onResize();
}

void ColFloorApp::drawScene()
{
	BaseDX::drawScene();
...
/*
  ...
  */
  ...
  //  Update camera
  GlobalGame::gCam3dPerson->updateTargetParameters( GlobalGame::gShip->xWS, GlobalGame::gShip->getvWS() );  
  GlobalGame::gCam3dPerson->updateCamera();

  //  Update application vectors

  bool  shipBaseChanged = false;
  if ( GlobalGame::gShip->hasUpwardsNormalOfContact() )
  {
    mLastNormalOfContact = GlobalGame::gShip->upwardsNormalOfContact();
    shipBaseChanged = true;
  }

  if ( !DxMath::equalsZero( GlobalGame::gShip->getvWS() ) ) 
  {
    D3DXVECTOR3 v = GlobalGame::gShip->getvWS();    
  ...
  if ( shipBaseChanged )
  {
    ...
    GlobalGame::gShipTForceForw->setApplicationVector ( mLastVelocityNonZero );
    GlobalGame::gShipTForceBack->setApplicationVector ( -1.0f*mLastVelocityNonZero );
    GlobalGame::gShipTForceLeft->setApplicationVector ( mLastCrossProd );
    GlobalGame::gShipTForceRight->setApplicationVector( -1.0f*mLastCrossProd );
  }
